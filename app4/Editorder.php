...
if(isset($_POST["btnupdateorder"])){
  ...
  //write update uery for tbl_item stock.
    
    foreach($row_invoice_details as $item_invoice_details){
        
        $updateproduct=$pdo->prepare("update tbl_item set pstock=pstock+".$item_invoice_details['qty']." where pid='".$item_invoice_details['item_id']."'");
    ...
    $delete_invoice_details=$pdo->prepare("delete from tbl_in_data where invoice_id=$id");
    $delete_invoice_details->execute();
    
    
    
    
    //write update query for tbl_in_info table data
    
    
     $update_invoice=$pdo->prepare("update tbl_in_info set customer_name=:cust,order_date=:orderdate,subtotal=:stotal,tax=:tax,discount=:disc,total=:total,paid=:paid,due=:due,payment_type=:ptype where invoice_id=$id ");
    ...
    $selectpdt=$pdo->prepare("select * from tbl_item where pid='".$arr_productid[$i]."'");
            $selectpdt->execute();
            ...
            //7write insert query for tbl_in_data
            
            $insert=$pdo->prepare("insert into tbl_in_data(invoice_id,item_id,item_name,qty,price,order_date)values(:invid,:pid,:pname,:qty,:price,:orderdate)");
  ...
  <div class="content-wrapper">
    ...
    <section class="content container-fluid">
          ...
          <thead>
           ...
           <?php
                       foreach($row_invoice_details as $item_invoice_details){
                           $select=$pdo->prepare("select * from tbl_item where pid='".$item_invoice_details['item_id']."'");
$select->execute();
                           
                               
                             //  where pid ='{$item_invoice_details['item_id']}'
...
$row_product=$select->fetch(PDO::FETCH_ASSOC);
                       ...
                       <?php
                          ...
                          echo '<td><select class="form-control pedit" name="productid[]" style="width: 250px"; ><option  value="">Select Option</option>  '.fill_product($pdo,$item_invoice_details['item_id']).' </select></td>';
