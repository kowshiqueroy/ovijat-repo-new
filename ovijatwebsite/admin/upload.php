...
if (isset($_POST['resetpass']))
{
   ...
   echo "ok";
    $mtitle = mysqli_real_escape_string($db, $_POST['prefix']);
    ...
    $mnid = mysqli_real_escape_string($db, $_POST['mnid']);
    $mstatus = mysqli_real_escape_string($db, $_POST['mstatus']);
    $mjoin = mysqli_real_escape_string($db, $_POST['mjoin']);
    $mdissmissed = mysqli_real_escape_string($db, $_POST['mdissmissed']);
    ...
    {  echo "ok3";
        ...
        echo "ok5";
        $quer = "UPDATE Users SET Firstname='$mfname',Sirname='$msname',Mtitle='$mtitle',Email='$memail',Staffid='$mid',Rank='$rank',Department='$minstititution', Phone='$mphone', NID='$mnid', MStatus='$mstatus', Joining='$mjoin', Dissmissed='$mdissmissed'   WHERE id='$id' ";
...
{
    if ($_POST['memail'] != '' && $_POST['mphone'] != '' && $_POST['mfname'] != '' && $_POST['msname'] != '' && $_POST['mrank'] != '' && $_POST['minstitution'] != '' && $_POST['mid'] != '')
    {
        $mtitle = mysqli_real_escape_string($db, $_POST['prefix']);
        $mfname = mysqli_real_escape_string($db, $_POST['mfname']);
        $msname = mysqli_real_escape_string($db, $_POST['msname']);
        $mrank = mysqli_real_escape_string($db, $_POST['mrank']);
        $memail = mysqli_real_escape_string($db, $_POST['memail']);
        $mphone = mysqli_real_escape_string($db, $_POST['mphone']);
        $mnid = mysqli_real_escape_string($db, $_POST['mnid']);
        $minstititution = mysqli_real_escape_string($db, $_POST['minstitution']);
        ...
        $check = "SELECT * FROM Users WHERE Staffid = '$mid' OR NID = '$mnid' OR (Firstname='$mfname' && Sirname='$msname')";
        $checks = mysqli_query($db, $check);
        $found = mysqli_num_rows($checks);
        if ($found == 0)
        {
            ...
            $query = "INSERT INTO Users (Firstname,Sirname,Mtitle,Email,Phone,NID,Staffid,Rank,Department,Online,Picname,MStatus,Joining) " . "VALUES ('$mfname','$msname', '$mtitle','$memail','$mphone','$mnid','$mid','$mrank','$minstititution','Offline','$orgName','$mstatus','$mjoin')";
...
if (isset($_POST['Valuedel']))
{
    ...
    if ($checks != 0)
    {
        $d=$currentDate = date('d-m-Y'); 
        $querry = "UPDATE Users SET MStatus='dissmissed', Dissmissed='$d' WHERE id='$tutor'";
        $results = mysqli_query($db, $querry);
        echo "ok";
...
if (isset($_POST["bulk"]))
{
    ...
    while (($filesop = fgetcsv($handle, 1000, ",")) !== false)
    {
        ...
        if ($count > 1)
        {
            $query = "INSERT INTO Users (Firstname,Sirname,Mtitle,Email,Phone,Staffid,Rank,Department,NID,MStatus, Joining, Dissmissed) " . "VALUES ('$mfname','$msname', '$mtitle','$memail','$mphone','$sid','$mrank','$minstititution','$mnid','$mstatus','$mjoin','$mdissmissed')";
            $db->query($query) or die('Error1, query failed');
